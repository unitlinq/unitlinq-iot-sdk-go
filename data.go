...
func (c *Client) PushEnergyData(data EnergyStruct) Token {
	...
	token = c.MQTT.Publish("device/data/energy/"+c.ClientID.String(), 1, false, encoded)
	return token
}
...
func (c *Client) PushFloatNP(datapoint NodeParamFloat) Token {
	...
	token := c.MQTT.Publish("device/data/param/"+c.ClientID.String(), 1, false, encoded)
	return token
}
...
func (c *Client) EnergyGateWay(data EnergyStruct, node uuid.UUID) Token {
	...
	token = c.MQTT.Publish("device/"+c.GetClientID()+"/gateway/energy", 1, false, encoded)
	return token
}
...
func (c *Client) NPFloatGateWay(data NodeParamFloat, node uuid.UUID) Token {
	...
	token := c.MQTT.Publish("device/"+c.GetClientID()+"/gateway/param", 1, false, encoded)
	return token
}
